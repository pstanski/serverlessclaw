...
| **Coder Agent**          | `High-Power` | Writes code, validates deployments, and enforces evolution patch outputs (including fallback extraction from `PATCH_START`/`PATCH_END` blocks). See [`packages/core/agents/coder.ts`](../../packages/core/agents/coder.ts). |
