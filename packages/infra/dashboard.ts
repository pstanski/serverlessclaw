  ...
  const dashboard = new sst.aws.Nextjs('MissionControl', {
    ...
    buildCommand: `mkdir -p src/extensions/project && find src/extensions/ -mindepth 1 -maxdepth 1 ! -name 'index.ts' ! -name 'messages' ! -name 'project' -exec rm -rf {} + && ${
      extSource
        ...
        : ''
    }NODE_OPTIONS=--max-old-space-size=6144 npx open-next build && find .open-next -name "*.map" -delete && cd .open-next/server-functions/default && rm -rf node_modules && npm install --production --no-package-lock --ignore-scripts && rm -rf node_modules/@aws-sdk node_modules/sst node_modules/typescript node_modules/prettier node_modules/eslint node_modules/puppeteer* node_modules/tree-sitter* node_modules/esbuild node_modules/@swc/core node_modules/rimraf node_modules/glob node_modules/caniuse-lite && find . -type d -name ".aiready" -exec rm -rf {} + && find . -type d -name ".turbo" -exec rm -rf {} + && rm -rf .git .github .husky docs makefiles scripts e2e reports apps/dashboard/src packages/*/src coverage tsconfig.tsbuildinfo`,
    server: {
