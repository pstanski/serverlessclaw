  ...
  const dashboard = new sst.aws.Nextjs('MissionControl', {
    ...
    buildCommand: `mkdir -p src/extensions/project && find src/extensions/ -mindepth 1 -maxdepth 1 ! -name 'index.ts' ! -name 'messages' ! -name 'project' -exec rm -rf {} + && ${
      extSource
        ...
        : ''
    }NODE_OPTIONS=--max-old-space-size=6144 npx open-next build && find .open-next -name "*.map" -delete && cd .open-next/server-functions/default && find . -type d -name ".aiready" -exec rm -rf {} + && find . -type d -name ".turbo" -exec rm -rf {} + && find . -type d -name "cache" -exec rm -rf {} + && find . -name "*.map" -delete && rm -rf .git .github .husky docs makefiles scripts e2e reports apps/dashboard/src packages/*/src coverage tsconfig.tsbuildinfo`,
    server: {
