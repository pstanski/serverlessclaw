...
describe('QA Agent — REOPEN cap and HITL escalation', () => {
  ...
  it('should set gaps to DONE and not escalate on SUCCESS (auto mode)', async () => {
    ...
    expect(memoryMocks.updateGapStatus).toHaveBeenCalledWith(
      'GAP#1001',
      GapStatus.DONE,
      expect.anything()
    );
  ...
  it('should REOPEN gap and increment attempt count on REOPEN status below cap', async () => {
    ...
    expect(memoryMocks.incrementGapAttemptCount).toHaveBeenCalledWith(
      'GAP#1001',
      expect.anything()
    );
    expect(memoryMocks.updateGapStatus).toHaveBeenCalledWith(
  ...
  it('should handle lock acquisition failure during SUCCESS path', async () => {
    ...
    expect(memoryMocks.updateGapStatus).not.toHaveBeenCalledWith(
      'GAP#1001',
      GapStatus.DONE,
      expect.anything()
    );
