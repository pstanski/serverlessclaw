...
async function main() {
  ...
  for (const item of items) {
    console.log(`--------------------------------------------------`);
    console.log(`🕒 Timestamp: ${item.timestamp} | NodeId: ${item.nodeId} | Agent: ${item.agentId || 'N/A'}`);
    ...
    if (item.input) {
      console.log(`📥 Input:`, typeof item.input === 'string' ? item.input.slice(0, 300) : JSON.stringify(item.input).slice(0, 300));
    }
    if (item.output) {
      console.log(`📤 Output:`, typeof item.output === 'string' ? item.output.slice(0, 300) : JSON.stringify(item.output).slice(0, 300));
    }
  }
}
